...
public class FilterAppController {
	...
	/**
	 * @param arr Array in 1-dimensionaler Form
	 * @param kernelW Kernelbreite
	 * @param kernelH Kernelhöhe
	 * @param width Bildbreite
	 * @param height Bildhöhe
	 * @param index Der Index, um aus dem jeweils sortierten Nachbararray den neuen Pixelwert auszusuchen
	 * @return Das fertige Bild
	 */

	static int[] applyFilter(int[] arr, int kernelW, int kernelH, int width, int height, int index) {
		int[] result = new int[arr.length];
		int i = 0;
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				int[] neighbourVals = new int[kernelW * kernelH]; //speichert Pixelwerte in der Umgebung
				int j = 0;
				for (int dx = -kernelW / 2; dx <= kernelW / 2; dx++) {
					for (int dy = -kernelH / 2; dy <= kernelH / 2; dy++) {
						int curX = x + dx;
						int curY = y + dy;
						if (0 < curX && curX < width && 0 < curY && curY < height) //Falls wir im Bild sind
							neighbourVals[j++] = arr[curY * width + curX]; //Wert auslesen
						else neighbourVals[j++] = 0xff888888; //Sonst Randbehandlung mit konst. Wert (grau)
					}
				}
				Arrays.sort(neighbourVals);
				result[i++] = neighbourVals[index]; //Per vom aufrufenden Filter gegebenen Index den neuen Pixelwert festlegen
			}
		}
		return result;
	}
